...
import 'package:cropmodel/core/shared/last_position_suggestion.dart';
...
import 'package:cropmodel/features/RestaurantDetails/presentaion/UI/restaurant_details.dart';
...
class _homepageState extends State<homepage> {
  ...
  Future<void> _checkLastPosition() async {
    final hasPosition = await _historyService.hasSavedPosition();
    if (mounted) {
      setState(() {
        _showLastPosition = hasPosition;
      });
    }
  }

  void _dismissLastPosition() {
    setState(() {
      _showLastPosition = false;
    });
  }

  Future<void> _navigateToLastPosition() async {
    final history = await _historyService.getLastPosition();
    final routeName = history['routeName'];
    final routeData = history['routeData'];

    if (!mounted) return;

    _dismissLastPosition();
    ...
    switch (routeName) {
      ...
      case 'restaurant_details':
        if (routeData != null) {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => RestaurantDetailsScreen(restaurantId: routeData),
            ),
          );
        }
        break;
      case 'menu':
        if (routeData != null) {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => MenuScreen(restaurantId: routeData),
            ),
          );
        }
        break;
      case 'cart':
        Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => const CartPresenter()),
        );
        break;
      case 'room':
        Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => RoomPresenter()),
        );
        break;
      case 'room_details':
        if (routeData != null) {
      ...
      case 'profile':
        Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => ProfilePresenter()),
        );
        break;
      case 'my_rooms':
        Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => const MyRoomsPresenter()),
        );
        break;
      default:
        break;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: ListView(
          children: [
            Padding(
              padding: EdgeInsetsDirectional.only(start: 20.w, top: 16.h),
              child: Row(
                children: [
            ...
            Row(
              children: [
                ...
                const Spacer(),
                Padding(
                  padding: EdgeInsetsDirectional.only(top: 16.h, end: 20.w),
                  child: InkWell(
                    onTap: () {
                      Navigator.push(
                        context,
                    ...
                    ? Column(
                        children: [
                          ...
                          Padding(
                            ...
                            child: Text(
                              "No_previous_orders_yet_Start_Your_frist_order"
                                  .tr(),
            ...
            if (_showLastPosition)
              LastPositionSuggestion(
                onDismiss: _dismissLastPosition,
                onGoBack: _navigateToLastPosition,
              ),
